...
# Redirection des logs : toutes les sorties stdout et stderr sont envoyées
# vers le fichier de log et affichées simultanément sur le terminal.
...
# =============================================================================
# Fonctions de vérification des prérequis essentiels (communes à plusieurs scripts)
# =============================================================================
check_whiptail() {
    if ! command -v whiptail &> /dev/null; then
        print_step "Installation de 'whiptail'..."
        if ! sudo apt update -qq > /dev/null 2>&1; then
             show_error_and_return_to_menu "Échec de la mise à jour des dépôts APT avant l'installation de whiptail."
             return 1
        fi
        if ! sudo apt install -y whiptail -qq > /dev/null 2>&1; then
            show_error_and_return_to_menu "Échec de l'installation de whiptail. Le script ne peut pas continuer sans whiptail."
            return 1
        else
...
check_bc() {
    if ! command -v bc &> /dev/null; then
        ...
        if ! sudo apt update -qq > /dev/null 2>&1; then
             show_error_and_return_to_menu "Échec de la mise à jour des dépôts APT avant l'installation de bc."
             return 1
        fi
        if ! sudo apt install -y bc -qq > /dev/null 2>&1; then
            print_error "Échec de l'installation de bc. Certains calculs pourraient être imprécis."
        else
...
check_curl() {
    if ! command -v curl &> /dev/null; then
        ...
        if ! sudo apt update -qq > /dev/null 2>&1; then
             show_error_and_return_to_menu "Échec de la mise à jour des dépôts APT avant l'installation de curl."
             return 1
        fi
        if ! sudo apt install -y curl -qq > /dev/null 2>&1; then
            show_error_and_return_to_menu "Échec de l'installation de curl. Le script ne peut pas récupérer les versions de GLPI."
            return 1
        else
...
# =============================================================================
# Logique Principale pour la Nouvelle Installation
# =============================================================================
main() {
    display_header
    print_step "Démarrage de la nouvelle installation GLPI..."

    # Pré-vérifications critiques (arrêt si échec)
    check_whiptail || exit 1
    check_bc || true # bc n'est pas critique
    check_curl || exit 1
    ...
    secure_glpi_web # Sécurisation post-installation
    restart_apache

    whiptail --title "Installation GLPI Réussie" --msgbox "GLPI a été installé avec succès.\n\nAccédez à votre GLPI via : http://${fqdn}\n\nIdentifiants par défaut : glpi / glpi" 15 70
}
